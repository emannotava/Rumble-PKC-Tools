...
class PiiPropDataJSONGUI:
	def __init__(self, root):
		...
		self._sidebar_icon_after_id = None
		self._sidebar_icon_queue = []
	...
	def build_ui(self):
		...
		style.configure(self.sidebar_tree_style, rowheight=24)
	...
	def refresh_sidebar(self):
		...
		try:
			self.cancel_sidebar_icon_loading()
		...
		self.schedule_sidebar_icon_loading()
	...
	def cancel_sidebar_icon_loading(self):
		after_id = self._sidebar_icon_after_id
		self._sidebar_icon_after_id = None
		if after_id:
			try:
	...
	def schedule_sidebar_icon_loading(self):
		self.cancel_sidebar_icon_loading()
		if not self.document or not self.sidebar_item_to_index:
			return
		self._sidebar_icon_queue = list(self.sidebar_item_to_index.items())
		self._sidebar_icon_after_id = self.root.after(1, self.process_sidebar_icon_batch)

	def process_sidebar_icon_batch(self):
		self._sidebar_icon_after_id = None
		if not self.document or not self._sidebar_icon_queue:
			return
		entries = self.document.get("entries", [])
		batch_size = 20
		processed = 0
		while self._sidebar_icon_queue and processed < batch_size:
			item_id, actual_index = self._sidebar_icon_queue.pop(0)
			if not self.entry_tree.exists(item_id):
				continue
			if not (0 <= actual_index < len(entries)):
				continue
			entry = entries[actual_index]
			species_id, form_id = self.get_species_and_form_ids(entry)
			icon_path = self.find_species_icon_path(species_id, form_id, kind="side")
			image = self.load_icon_image(icon_path, max_dim=20)
			if image is not None:
				self.sidebar_icon_images[item_id] = image
				self.entry_tree.item(item_id, image=image)
			else:
				self.sidebar_icon_images.pop(item_id, None)
				self.entry_tree.item(item_id, image="")
			processed += 1
		if self._sidebar_icon_queue:
			self._sidebar_icon_after_id = self.root.after(1, self.process_sidebar_icon_batch)
	...
	def select_entry(self, index):
		...
		try:
			...
			self.populate_form(entries[index])
		finally:
			self._selecting_entry = False
	...
	def populate_form(self, entry):
		...
		for field_name in NUMERIC_FIELD_NAMES:
			self.field_vars[field_name].set(
				self.format_numeric_display(fields.get(field_name), percent=field_name in PERCENT_FIELD_NAMES)
			)
		...
		self.block_info_label.config(
			text=f"Block {entry.get('block_index')} | Setter: {entry.get('setter')} | Key: {entry.get('key')}"
		)
	...
	def get_species_and_form_ids(self, entry):
		...
		form_id = None if form_value in (None, "") else safe_int(form_value)
	...
	def update_species_icon(self):
		...
		self.species_icon_image = self.load_icon_image(icon_path, max_dim=128)
	...
	def find_species_icon_path(self, species_id, form_id, kind):
		candidates = []
		if species_id is not None and form_id is not None:
	...
	def save_current_entry_to_document(self, silent=False):
		...
		try:
			...
			form_text = self.form_var.get().strip()
			form_value = None if not form_text else int(self.normalize_number(form_text))

			entry["species"] = self.species_id_to_symbol(species_id)
			entry["form"] = form_value
			entry["key"] = entry["species"] if form_value is None else f"{entry['species']},{form_value}"

			fields = entry.setdefault("fields", {})
			for field_name in NUMERIC_FIELD_NAMES:
				label_text = next(label for group in FIELD_LAYOUT for label, name in group if name == field_name)
				fields[field_name] = self.parse_numeric_field(label_text, self.field_vars[field_name].get(), percent=field_name in PERCENT_FIELD_NAMES)
			...
			self.status_var.set(f"Updated entry {entry.get('block_index')}.")
			return True
		except Exception as exc:
			if not silent:
