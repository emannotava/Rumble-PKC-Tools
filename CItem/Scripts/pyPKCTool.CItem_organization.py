...
TOOL_DESCRIPTION = "Edit CItem_organization.pkc via semantic JSON."
FORMAT_VERSION = "citem-organization-semantic-json-1"
...
TERMINAL_VISUAL_PCS = {
	...
	"model_brres": 0x0985,
	"fall_motion": 0x09E1,
	"show_after_fall": 0x09E5,
}

UI_FLOW_PCS = {
	"low_battery_pad_index": 0x0967,
	"pause_se_test_id": 0x09A5,
	"balloon_slot": 0x09C5,
	"init_balloon_mid": 0x09C6,
	"balloon_y_adjust": 0x09CC,
	"visibility_flag": 0x09D1,
	"show_when_flag_missing": 0x09D6,
	"select_prompt_mid": 0x09F6,
	"pii_list_type": 0x0A07,
}
...
def export_semantic(document):
	...
	return {
		...
		"terminal_visuals": {
			...
			"show_after_fall": read_push_int(pc_to_instruction, TERMINAL_VISUAL_PCS["show_after_fall"]),
		},
		"ui_flow": {
			"low_battery_pad_index": read_push_int(pc_to_instruction, UI_FLOW_PCS["low_battery_pad_index"]),
			"pause_se_test_id": read_push_int(pc_to_instruction, UI_FLOW_PCS["pause_se_test_id"]),
			"balloon_slot": read_push_int(pc_to_instruction, UI_FLOW_PCS["balloon_slot"]),
			"init_balloon_mid": read_push_int(pc_to_instruction, UI_FLOW_PCS["init_balloon_mid"]),
			"balloon_y_adjust": read_push_int(pc_to_instruction, UI_FLOW_PCS["balloon_y_adjust"]),
			"visibility_flag": read_push_int(pc_to_instruction, UI_FLOW_PCS["visibility_flag"]),
			"show_when_flag_missing": read_push_int(pc_to_instruction, UI_FLOW_PCS["show_when_flag_missing"]),
			"select_prompt_mid": read_push_int(pc_to_instruction, UI_FLOW_PCS["select_prompt_mid"]),
			"pii_list_type": read_push_int(pc_to_instruction, UI_FLOW_PCS["pii_list_type"]),
		},
	}
...
def apply_semantic(document, semantic):
	...
	for key in ("model_brres", "fall_motion"):
		write_push_str_group(string_entries, pc_to_string_index, [TERMINAL_VISUAL_PCS[key]], [semantic["terminal_visuals"][key]])
	write_push_int(pc_to_instruction, TERMINAL_VISUAL_PCS["show_after_fall"], semantic["terminal_visuals"]["show_after_fall"])
	for key in ("low_battery_pad_index", "pause_se_test_id", "balloon_slot", "init_balloon_mid", "balloon_y_adjust", "visibility_flag", "show_when_flag_missing", "select_prompt_mid", "pii_list_type"):
		write_push_int(pc_to_instruction, UI_FLOW_PCS[key], semantic["ui_flow"][key])
	return document
