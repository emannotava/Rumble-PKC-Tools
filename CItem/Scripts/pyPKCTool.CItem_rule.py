...
TOOL_DESCRIPTION = "Edit CItem_rule.pkc via semantic JSON."
FORMAT_VERSION = "citem-rule-semantic-json-1"
...
UI_FLOW_PCS = {
	...
	"show_on_init": 0x097C,
	"dir_deg": 0x0980,
	"pause_se_test_id": 0x09A1,
	"balloon_slot": 0x09C1,
	"init_balloon_mid": 0x09C2,
	"player_message_mid": 0x09C9,
	"gating_flag": 0x09E5,
	"one_time_flag": 0x09E8,
	"set_one_time_flag": 0x09F5,
	"message_wait_frames": 0x09FA,
}
...
def read_string_list(string_entries, pc_to_string_index, pcs):
	...
	for pc in pcs:
		...
		values.append(string_entries[pc_to_string_index[pc]])
	return values
...
def export_semantic(document):
	...
	return {
		...
		"ui_flow": {
			...
			"show_on_init": read_push_int(pc_to_instruction, UI_FLOW_PCS["show_on_init"]),
			"dir_deg": read_push_float(pc_to_instruction, UI_FLOW_PCS["dir_deg"]),
			...
			"player_message_mid": read_push_int(pc_to_instruction, UI_FLOW_PCS["player_message_mid"]),
			"gating_flag": read_push_int(pc_to_instruction, UI_FLOW_PCS["gating_flag"]),
			"one_time_flag": read_push_int(pc_to_instruction, UI_FLOW_PCS["one_time_flag"]),
			"set_one_time_flag": read_push_int(pc_to_instruction, UI_FLOW_PCS["set_one_time_flag"]),
			"message_wait_frames": read_push_int(pc_to_instruction, UI_FLOW_PCS["message_wait_frames"]),
		},
	}
...
def apply_semantic(document, semantic):
	...
	for key in ("low_battery_pad_index", "show_on_init", "pause_se_test_id", "balloon_slot", "init_balloon_mid", "player_message_mid", "gating_flag", "one_time_flag", "set_one_time_flag", "message_wait_frames"):
		write_push_int(pc_to_instruction, UI_FLOW_PCS[key], semantic["ui_flow"][key])
	write_push_float(pc_to_instruction, UI_FLOW_PCS["dir_deg"], semantic["ui_flow"]["dir_deg"])
	return document
