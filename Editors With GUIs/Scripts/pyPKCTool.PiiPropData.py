...
def collect_entry_values(entry: dict) -> List[float]:
	...
	if not isinstance(fields, dict):
		raise ValueError("Entry must have a fields object")
	values = []
	for field_name in FIELD_NAMES:
		...
		if lookup_name not in fields:
			raise ValueError(f"Entry is missing field {field_name!r}")
...
def build_export_document(pkc_path: str) -> dict:
	...
	for block in blocks:
		...
		exported_form = block["form"] if block["form"] is not None else 0
		entries.append({
	...
	return {
		...
		"notes": {
			"editing": [
				...
				"form exports as 0 for all-form/base blocks instead of null.",
				...
				"For base/all-form entries, keep form as 0 and setter as method_pprSetAllForAllFormno.",
...
def normalize_species_and_form(entry: dict, for_existing_form: Optional[int], entry_pos: Optional[int] = None) -> Tuple[int, Optional[int]]:
	species = species_value_to_id(entry["species"])
	if not isinstance(species, int):
		raise ValueError(f"Entry at index {entry_pos}: species must resolve to an integer")
	...
	if for_existing_form is None:
		...
		if not isinstance(form, int):
			raise ValueError(f"Entry at index {entry_pos}: form must be an integer")
		if form != 0:
			raise ValueError(f"Entry at index {entry_pos}: base/all-form block must keep form as 0")
		return species, None

	if form_value is None:
		raise ValueError(f"Entry at index {entry_pos}: form-specific block must keep a numeric form")
	form = normalize_number(form_value)
	if not isinstance(form, int):
		raise ValueError(f"Entry at index {entry_pos}: form must be an integer")
	return species, form
...
def infer_or_validate_new_entry_setter(entry: dict, species: int, form: Optional[int]) -> str:
	...
	if setter == "method_pprSetAllForAllFormno" and form not in (None, 0):
		raise ValueError(f"New entry for species {species}, form {form}: method_pprSetAllForAllFormno requires form=0")
...
def synthesize_new_block_instructions(entry: dict) -> Tuple[str, List[dict], Tuple[int, Optional[int]]]:
	species = species_value_to_id(entry["species"])
	if not isinstance(species, int):
		raise ValueError("New entry species must resolve to an integer")
	...
	if form_raw is not None:
		...
		if not isinstance(form, int):
			raise ValueError("New entry form must be an integer when provided")

	setter = infer_or_validate_new_entry_setter(entry, species, form)
	effective_form = None if setter == "method_pprSetAllForAllFormno" else form
	values = collect_entry_values(entry)

	new_instructions = []
	new_instructions.append(make_push_instruction(species))
	if effective_form is not None:
		new_instructions.append(make_push_instruction(effective_form))
	new_instructions.extend(make_call_ext_1_instruction("GetPiiProp", 2 if effective_form is not None else 1))
	for value in values:
	...
	return setter, new_instructions, (species, effective_form)
...
def apply_json_to_document(pkc_document: dict, edit_document: dict) -> dict:
	accepted_formats = {"piiprop-edit-json-2", FORMAT_VERSION}
	if edit_document.get("format") not in accepted_formats:
	...
	for entry_pos, entry in enumerate(entries):
		...
		is_existing = entry_pos < len(blocks)
		if is_existing:
			block = blocks[entry_pos]

			species, form = normalize_species_and_form(entry, block["form"], entry_pos)
			setter = entry.get("setter")
			if setter != block["setter"]:
				raise ValueError(f"Entry at index {entry_pos}: setter changed from {block['setter']} to {setter}")
			values = collect_entry_values(entry)
			...
			if key in final_keys:
				raise ValueError(
					f"Duplicate species/form key after edits: {species}"
					if form is None
					else f"Duplicate species/form key after edits: {species},{form}"
				)
			final_keys[key] = ("existing", entry_pos)
			continue
		...
		if key in final_keys:
			species, form = key
			raise ValueError(
				f"Duplicate species/form key in added blocks: {species}"
				if form is None
				else f"Duplicate species/form key in added blocks: {species},{form}"
			)
		final_keys[key] = ("new", entry_pos)
	...
	if len(entries) < len(blocks):
		raise ValueError(
			f"JSON does not contain all original blocks. Expected at least {len(blocks)} entries, got {len(entries)}."
		)
