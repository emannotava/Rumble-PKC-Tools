...
FORMAT_VERSION = "cevent-battleroyal-defeat-semantic-json-1"
...
DEFEAT_INT_PCS = {
	"start_se_id": 0x0912,
	"post_effect_frames": 0x0915,
	"post_effect_id": 0x0918,
	"animation_id": 0x094B,
	"effect_direction_deg": 0x0950,
	"initial_rotation_state": 0x095D,
	"rotation_slot_reset": 0x095F,
	"rotation_guard_count": 0x0962,
	"normalize_target_deg": 0x0974,
	"normalize_compare_deg": 0x0979,
	"setdir_zero_value": 0x098E,
	"yield_reset_value": 0x0994,
	"final_se_test_id": 0x099A,
	"delay_seconds": 0x099D,
	"delay_frames": 0x099E,
	"despawn_steps": 0x09A2,
	"despawn_start_offset": 0x09A3,
	"position_divisor_x": 0x09B7,
	"position_divisor_y": 0x09C1,
	"position_divisor_z": 0x09CB,
}
DEFEAT_FLOAT_PCS = {
	"rotation_step_negative_deg": 0x097C,
	"rotation_step_positive_deg": 0x0982,
}
...
def export_semantic(document):
	...
	return {
		...
		"defeat_sequence": {
			"integers": {key: read_push_int(pc_to_instruction, pc) for key, pc in DEFEAT_INT_PCS.items()},
			"floats": {key: read_push_float(pc_to_instruction, pc) for key, pc in DEFEAT_FLOAT_PCS.items()},
		},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pc in DEFEAT_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["defeat_sequence"]["integers"][key])
	for key, pc in DEFEAT_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["defeat_sequence"]["floats"][key])
	return document
...
def build_parser():
	parser = argparse.ArgumentParser(description="Edit CEvent_battleroyal_defeat.pkc via semantic JSON.")
