...
TOOL_DESCRIPTION = 'Edit CEvent_lose.pkc via semantic JSON.'
FORMAT_VERSION = 'cevent-lose-semantic-json-1'
...
LOSS_INT_PCS = {
	"hide_enemy_info_delay_frames": 0x0905,
	"bgm_id": 0x090C,
	"initial_post_effect_id": 0x090F,
	"notice_display_type": 0x091A,
	"secondary_post_effect_delay_frames": 0x091D,
	"secondary_post_effect_id": 0x0920,
	"tertiary_post_effect_delay_frames": 0x0923,
	"tertiary_post_effect_id": 0x0928,
	"map_status_flag": 0x092C,
	"map_status_mode": 0x0933,
	"check_flag_primary": 0x093C,
	"set_flag_on_lose": 0x093F,
	"make_stage_id_tail": 0x0947,
	"stage_type_compare": 0x094C,
	"check_flag_secondary": 0x094F,
	"stat_add_primary_id": 0x0952,
	"stat_add_primary_value": 0x0953,
	"end_stage_arg_a": 0x0957,
	"end_stage_arg_b": 0x0958,
	"stat_add_secondary_id": 0x095C,
	"stat_add_secondary_value": 0x095D,
}
...
def export_semantic(document):
	...
	return {
		...
		"loss_sequence": {key: read_push_int(pc_to_instruction, pc) for key, pc in LOSS_INT_PCS.items()},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pc in LOSS_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["loss_sequence"][key])
	return document
...
def command_summary(args):
	...
	print(f"Lose BGM ID: {semantic['loss_sequence']['bgm_id']}")
	print(f"Notice display type: {semantic['loss_sequence']['notice_display_type']}")
	print(f"Set flag on lose: {semantic['loss_sequence']['set_flag_on_lose']}")
