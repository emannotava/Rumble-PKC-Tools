...
TOOL_DESCRIPTION = "Edit CEvent_warppoint_open.pkc via semantic JSON."
FORMAT_VERSION = "cevent-warppoint-open-semantic-json-1"
...
MESSAGE_MID_PCS = [0x0904, 0x0909, 0x096D]
ACTIVATION_INT_PCS = {
	"register_slot": 0x0925,
	"register_value": 0x0926,
	"unlock_flag_id": 0x0957,
	"setflag_value": 0x095A,
	"effect_sound_se_id": 0x0962,
	"show_post_effect_id": 0x0965,
	"hide_post_effect_frames": 0x0968,
	"destroy_delay_frames": 0x097F,
}
ACTIVATION_FLOAT_PCS = {
	"spawn_offset_x": 0x094D,
	"spawn_offset_y": 0x094F,
	"spawn_offset_z": 0x0951,
	"spin_speed": 0x0953,
	"final_scale": 0x097B,
}
...
def export_semantic(document):
	...
	return {
		...
		"message_mids": read_int_list(pc_to_instruction, MESSAGE_MID_PCS),
		"activation_effects": {
			"integers": {key: read_push_int(pc_to_instruction, pc) for key, pc in ACTIVATION_INT_PCS.items()},
			"floats": {key: read_push_float(pc_to_instruction, pc) for key, pc in ACTIVATION_FLOAT_PCS.items()},
		},
	}
...
def apply_semantic(document, semantic):
	...
	write_int_list(pc_to_instruction, MESSAGE_MID_PCS, semantic["message_mids"], "message_mids")
	for key, pc in ACTIVATION_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["activation_effects"]["integers"][key])
	for key, pc in ACTIVATION_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["activation_effects"]["floats"][key])
	return document
...
def command_summary(args):
	...
	print(f"Message count: {len(semantic['message_mids'])}")
	print(f"Unlock flag: {semantic['activation_effects']['integers']['unlock_flag_id']}")
	print(f"Final scale: {semantic['activation_effects']['floats']['final_scale']}")
