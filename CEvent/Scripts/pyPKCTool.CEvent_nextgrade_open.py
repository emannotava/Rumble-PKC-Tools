...
TOOL_DESCRIPTION = 'Edit CEvent_nextgrade_open.pkc via semantic JSON.'
FORMAT_VERSION = 'cevent-nextgrade-open-semantic-json-1'
...
GRADE_STRING_GROUPS = {
	"badge_b": [0x0977, 0x097F],
	"badge_a": [0x0982],
	"badge_s": [0x0985],
	"another_badge_c": [0x0988],
	"another_badge_b": [0x0996],
	"another_badge_a": [0x0999],
	"another_badge_s": [0x099C],
	"badge_ex": [0x099F],
}
GRADE_INT_PCS = {
	"intro_message_mid": 0x09AB,
	"image_message_mid": 0x09BE,
	"bonus_message_mid": 0x09F5,
	"grade_message_mid": 0x0A01,
	"outro_message_mid": 0x0A13,
	"start_se_id": 0x09BB,
	"bonus_check_flag": 0x09F1,
	"register_slot": 0x09E7,
	"register_value": 0x09E8,
}
GRADE_FLOAT_PCS = {
	"spawn_move_step": 0x09DF,
}
...
def export_semantic(document):
	...
	return {
		...
		"grade_badges": {key: read_push_str(string_entries, pc_to_string_index, pcs[0])[0] for key, pcs in GRADE_STRING_GROUPS.items()},
		"messages_and_flags": {
			"integers": {key: read_push_int(pc_to_instruction, pc) for key, pc in GRADE_INT_PCS.items()},
			"floats": {key: read_push_float(pc_to_instruction, pc) for key, pc in GRADE_FLOAT_PCS.items()},
		},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pcs in GRADE_STRING_GROUPS.items():
		write_push_str_group(string_entries, pc_to_string_index, pcs, [semantic["grade_badges"][key]] * len(pcs))
	for key, pc in GRADE_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["messages_and_flags"]["integers"][key])
	for key, pc in GRADE_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["messages_and_flags"]["floats"][key])
	return document
...
def command_summary(args):
	...
	print(f"Badge B image: {semantic['grade_badges']['badge_b']}")
	print(f"Intro message MID: {semantic['messages_and_flags']['integers']['intro_message_mid']}")
	print(f"Bonus check flag: {semantic['messages_and_flags']['integers']['bonus_check_flag']}")
