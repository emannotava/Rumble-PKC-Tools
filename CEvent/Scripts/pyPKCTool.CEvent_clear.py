...
TOOL_DESCRIPTION = 'Edit CEvent_clear.pkc via semantic JSON.'
FORMAT_VERSION = 'cevent-clear-semantic-json-1'
...
COMPLETION_STRING_GROUPS = {
	"blocker_motion": [0x099C],
	"warppoint_fall_motion": [0x09A7, 0x09AF],
	"ground_attr_path": [0x09C5],
}
COMPLETION_INT_PCS = {
	"notice_display_type": 0x0978,
	"bgm_id": 0x097B,
	"blocker_show_flag": 0x09A0,
	"warppoint_show_flag": 0x09AB,
	"motion_length_padding_frames": 0x09B3,
	"post_effect_id": 0x09B7,
	"post_effect_se_id": 0x09BA,
	"post_effect_hold_frames": 0x09BD,
	"hide_post_effect_hold_frames": 0x09C2,
}
COMPLETION_FLOAT_PCS = {
	"warp_step_scale": 0x0994,
	"cleanup_destroy_scale": 0x09CB,
}
...
def export_semantic(document):
	...
	return {
		...
		"completion_flow": {
			"strings": {key: read_push_str(string_entries, pc_to_string_index, pcs[0])[0] for key, pcs in COMPLETION_STRING_GROUPS.items()},
			"integers": {key: read_push_int(pc_to_instruction, pc) for key, pc in COMPLETION_INT_PCS.items()},
			"floats": {key: read_push_float(pc_to_instruction, pc) for key, pc in COMPLETION_FLOAT_PCS.items()},
		},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pcs in COMPLETION_STRING_GROUPS.items():
		write_push_str_group(string_entries, pc_to_string_index, pcs, [semantic["completion_flow"]["strings"][key]] * len(pcs))
	for key, pc in COMPLETION_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["completion_flow"]["integers"][key])
	for key, pc in COMPLETION_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["completion_flow"]["floats"][key])
	return document
...
def command_summary(args):
	...
	print(f"Notice display type: {semantic['completion_flow']['integers']['notice_display_type']}")
	print(f"BGM ID: {semantic['completion_flow']['integers']['bgm_id']}")
	print(f"Warp motion: {semantic['completion_flow']['strings']['warppoint_fall_motion']}")
