...
FORMAT_VERSION = "cevent-battleroyal-clear-semantic-json-1"
...
TRANSITION_FLOAT_PCS = {
	"offset_step_x": 0x093C,
	"offset_step_y": 0x093F,
	"base_scale": 0x0943,
	"negative_scale_numerator": 0x094A,
}
GAMEPLAY_INT_PCS = {
	"stage_clear_flag_threshold_round": 0x09C2,
	"stage_clear_flag_id": 0x09C5,
	"clear_map_type": 0x09CA,
	"clear_map_index": 0x09CC,
	"clear_round_flag_id": 0x09D8,
	"promotion_threshold_grade": 0x09EA,
	"promotion_flag_id": 0x09EC,
	"unlock_threshold_grade": 0x0A23,
	"unlock_advanced_difficulty": 0x0A2D,
	"unlock_advanced_grade": 0x0A27,
	"unlock_ex_difficulty": 0x0A39,
	"unlock_ex_grade": 0x0A3C,
}
...
def export_semantic(document):
	...
	return {
		...
		"gameplay": {key: read_push_int(pc_to_instruction, pc) for key, pc in GAMEPLAY_INT_PCS.items()},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pc in GAMEPLAY_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["gameplay"][key])
	return document
...
def build_parser():
	parser = argparse.ArgumentParser(description="Edit CEvent_battleroyal_clear.pkc via semantic JSON.")
