...
FORMAT_VERSION = "cevent-battleroyal-prize-semantic-json-1"
...
DIRECTION_SUFFIX_PCS = [0x0901, 0x0904, 0x0907, 0x090A, 0x090D, 0x0910, 0x0913, 0x0916, 0x0919]
TRANSITION_FLOAT_PCS = {"offset_step_x": 0x093C, "offset_step_y": 0x093F, "base_scale": 0x0943, "negative_scale_numerator": 0x094A}
PRIZE_FLOW_INT_PCS = {"reward_stat_id": 0x0984, "reward_bgm_id": 0x098B, "notice_id_first_half": 0x0998, "notice_id_second_half": 0x099C, "wait_after_notice_frames": 0x09A5, "money_register_slot": 0x09A8, "register_slot_12": 0x09AC, "register_slot_13": 0x09B0}
REWARD_ITEM_SCRIPT_PC = 0x09B4
REWARD_ITEM_SPAWN_OFFSET_Y_PC = 0x09B9
...
def export_semantic(document):
	...
	return {"format": FORMAT_VERSION, "source_basename": document.get("source_basename"),
		...
		"direction_suffixes": read_string_list(string_entries, pc_to_string_index, DIRECTION_SUFFIX_PCS),
		"layout_transition": {key: read_push_float(pc_to_instruction, pc) for key, pc in TRANSITION_FLOAT_PCS.items()},
		"prize_flow": {key: read_push_int(pc_to_instruction, pc) for key, pc in PRIZE_FLOW_INT_PCS.items()},
		"reward_item": {"reward_item_script": read_push_str(string_entries, pc_to_string_index, REWARD_ITEM_SCRIPT_PC)[0], "spawn_offset_y": read_push_float(pc_to_instruction, REWARD_ITEM_SPAWN_OFFSET_Y_PC)},
	}
...
def apply_semantic(document, semantic):
	...
	write_push_str_group(string_entries, pc_to_string_index, DIRECTION_SUFFIX_PCS, semantic["direction_suffixes"])
	for key, pc in TRANSITION_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["layout_transition"][key])
	for key, pc in PRIZE_FLOW_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["prize_flow"][key])
	write_push_str_group(string_entries, pc_to_string_index, [REWARD_ITEM_SCRIPT_PC], [semantic["reward_item"]["reward_item_script"]])
	write_push_float(pc_to_instruction, REWARD_ITEM_SPAWN_OFFSET_Y_PC, semantic["reward_item"]["spawn_offset_y"])
	return document
...
def build_parser():
	parser = argparse.ArgumentParser(description="Edit CEvent_battleroyal_prize.pkc via semantic JSON.")
