...
FORMAT_VERSION = "cevent-battleroyal-open-semantic-json-1"
...
LAYOUT_STRING_PCS = {"archive_path": 0x0903, "layout_name": 0x0904, "text_slot": 0x093C, "state_fadein": 0x0943, "state_fadeout": 0x094B, "state_off": 0x095A}
MESSAGE_INT_PCS = {"message_mid": 0x092C, "text_mid": 0x093D, "required_flag": 0x0937}
TIMING_INT_PCS = {"initial_wait_frames": 0x0918, "pre_message_hold_frames": 0x091B, "text_setup_delay_frames": 0x0929, "post_fadein_hold_frames": 0x0947, "fadeout_hold_frames": 0x0956}
PARAMETER_INT_PCS = {"layout_anchor_mode": 0x0907, "show_message_flag": 0x090C, "anchor_variant": 0x090D, "text_scroll_flag": 0x091E, "text_arg_a": 0x0921, "text_arg_b": 0x0922}
TIMING_FLOAT_PCS = {"layout_y_offset": 0x0908, "text_reveal_speed": 0x0925, "return_move_step": 0x0952}
...
def export_semantic(document):
	...
	return {"format": FORMAT_VERSION, "source_basename": document.get("source_basename"),
		...
		"message_window": {key: read_push_str(string_entries, pc_to_string_index, pc)[0] for key, pc in LAYOUT_STRING_PCS.items()},
		"message_ids": {key: read_push_int(pc_to_instruction, pc) for key, pc in MESSAGE_INT_PCS.items()},
		"timing": {"integers": {key: read_push_int(pc_to_instruction, pc) for key, pc in TIMING_INT_PCS.items()}, "floats": {key: read_push_float(pc_to_instruction, pc) for key, pc in TIMING_FLOAT_PCS.items()}},
		"parameters": {key: read_push_int(pc_to_instruction, pc) for key, pc in PARAMETER_INT_PCS.items()},
	}
...
def apply_semantic(document, semantic):
	...
	for key, pc in LAYOUT_STRING_PCS.items():
		write_push_str_group(string_entries, pc_to_string_index, [pc], [semantic["message_window"][key]])
	for key, pc in MESSAGE_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["message_ids"][key])
	for key, pc in TIMING_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["timing"]["integers"][key])
	for key, pc in TIMING_FLOAT_PCS.items():
		write_push_float(pc_to_instruction, pc, semantic["timing"]["floats"][key])
	for key, pc in PARAMETER_INT_PCS.items():
		write_push_int(pc_to_instruction, pc, semantic["parameters"][key])
	return document
...
def build_parser():
	parser = argparse.ArgumentParser(description="Edit CEvent_battleroyal_open.pkc via semantic JSON.")
